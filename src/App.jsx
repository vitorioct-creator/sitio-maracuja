...
import { loadShared, saveShared, supabase } from "./lib/storage";
import {
  signInWithUsername, fetchOwnProfile, buscarEmailMascarado, enviarCodigoRecuperacao,
  confirmarCodigoRecuperacao, definirNovaSenha, registrarLogin, atualizarPresenca, estaOnline,
  criarUsuario, atualizarUsuario, resetarSenhaUsuario, excluirUsuario,
} from "./lib/auth";
...
function LoginScreen({ onLoggedIn }) {
  const [modo, setModo] = useState("login"); // login | esqueci-usuario | esqueci-codigo | esqueci-senha | esqueci-ok
  ...
  const [recEmailMascarado, setRecEmailMascarado] = useState("");
  ...
  const [recCarregando, setRecCarregando] = useState(false);
  const [recErro, setRecErro] = useState("");

  const dispararErro = (msg) => {
    setErr(msg);
    setShakeErr(false);
    requestAnimationFrame(() => setShakeErr(true));
  };

  const salvarCredencialNoNavegador = async () => {
    try {
      if (window.PasswordCredential && formRef.current) {
        const cred = new window.PasswordCredential(formRef.current);
  ...
  const trySubmit = async (e) => {
    ...
    if (entrando) return;
    setEntrando(true);
    const { error } = await signInWithUsername(u.trim(), p);
    if (!error) {
      await salvarCredencialNoNavegador();
      onLoggedIn();
    } else {
      setEntrando(false);
      dispararErro(error);
  ...
  // Passo 1: usuário informa o login, sistema mostra e-mail mascarado e envia o código.
  const iniciarRecuperacao = async () => {
    setRecErro("");
    if (!recUsername.trim()) return setRecErro("Informe seu usuário.");
    setRecCarregando(true);
    const mascarado = await buscarEmailMascarado(recUsername.trim());
    if (!mascarado) {
      setRecCarregando(false);
      return setRecErro("Usuário não encontrado.");
    }
    const resultado = await enviarCodigoRecuperacao(recUsername.trim());
    setRecCarregando(false);
    if (resultado.error) return setRecErro(resultado.error);
    setRecEmailMascarado(mascarado);
    setModo("esqueci-codigo");
  };

  // Passo 2: confirma o código de 6 dígitos.
  const confirmarCodigo = async () => {
    setRecErro("");
    if (!recCodigo.trim()) return setRecErro("Informe o código recebido por e-mail.");
    setRecCarregando(true);
    const { data: email } = await supabase.rpc("get_email_by_username", { p_username: recUsername.trim() });
    const resultado = await confirmarCodigoRecuperacao(email, recCodigo.trim());
    setRecCarregando(false);
    if (resultado.error) return setRecErro(resultado.error);
  ...
  const salvarNovaSenha = async () => {
    setRecErro("");
    if (!recNovaSenha || recNovaSenha.length < 6) return setRecErro("A senha precisa ter pelo menos 6 caracteres.");
    if (recNovaSenha !== recConfirmar) return setRecErro("A confirmação não é igual à nova senha.");
    setRecCarregando(true);
    const resultado = await definirNovaSenha(recNovaSenha);
    setRecCarregando(false);
    if (resultado.error) return setRecErro(resultado.error);
  ...
  const voltarParaLogin = () => {
    setModo("login");
    setRecUsername(""); setRecEmailMascarado(""); setRecCodigo(""); setRecNovaSenha(""); setRecConfirmar(""); setRecErro("");
  };
  ...
  return (
    ...
    >
      <style>{`
        ...
        .shake-erro { animation: shakeErro 0.5s; }
        @keyframes girar { to { transform: rotate(360deg); } }
        .spinner-entrar { animation: girar 0.7s linear infinite; }
      `}</style>
      ...
      <div className="w-full max-w-sm">
          ...
          >
            <Field label="Usuário">
              <div className="relative">
                ...
                <input name="username" autoComplete="username" className={inputCls + " pl-9"} style={inputStyle} value={u} onChange={(e) => setU(e.target.value)} placeholder="Digite seu usuário" />
              </div>
            </Field>
            <Field label="Senha">
              <div className="relative">
                ...
                <input
                  name="password" autoComplete="current-password" type={mostrarSenha ? "text" : "password"}
                  className={inputCls + " pl-9 pr-9"} style={inputStyle} value={p} onChange={(e) => setP(e.target.value)} placeholder="••••••"
                />
                <button type="button" onClick={() => setMostrarSenha((s) => !s)} className="absolute right-3 top-1/2 -translate-y-1/2" tabIndex={-1}>
                  {mostrarSenha ? <EyeOff size={16} style={{ color: PALETTE.textDim }} /> : <Eye size={16} style={{ color: PALETTE.textDim }} />}
                </button>
              </div>
            </Field>
            {err && (
              <p key={err + String(shakeErr)} className={"text-sm " + (shakeErr ? "shake-erro" : "")} style={{ color: PALETTE.danger }}>{err}</p>
            )}
            <Btn type="submit" disabled={entrando} className="w-full">
              {entrando ? (
                <><span className="spinner-entrar" style={{ width: 14, height: 14, borderRadius: "50%", border: `2px solid ${PALETTE.polpaText}55`, borderTopColor: PALETTE.polpaText, display: "inline-block" }} /> Entrando...</>
              ) : (
                <>Entrar <ChevronRight size={16} /></>
              )}
            </Btn>
            <button type="button" onClick={() => setModo("esqueci-usuario")} className="w-full text-center text-xs" style={{ color: PALETTE.casca }}>
              Esqueci minha senha
            </button>
          </form>
        )}

        {modo === "esqueci-usuario" && (
          <div className="rounded-2xl p-6 space-y-4" style={{ background: PALETTE.card, border: `1px solid ${PALETTE.cardBorder}` }}>
            <p className="text-sm font-medium" style={{ color: PALETTE.text }}>Esqueci minha senha</p>
            <Field label="Seu usuário">
              <input className={inputCls} style={inputStyle} value={recUsername} onChange={(e) => setRecUsername(e.target.value)} />
            </Field>
            {recErro && <p className="text-sm" style={{ color: PALETTE.danger }}>{recErro}</p>}
            <Btn onClick={iniciarRecuperacao} disabled={recCarregando} className="w-full">{recCarregando ? "Enviando..." : "Enviar código"}</Btn>
            <button onClick={voltarParaLogin} className="w-full text-center text-xs" style={{ color: PALETTE.textDim }}>Voltar ao login</button>
          </div>
        )}

        {modo === "esqueci-codigo" && (
          <div className="rounded-2xl p-6 space-y-4" style={{ background: PALETTE.card, border: `1px solid ${PALETTE.cardBorder}` }}>
            <p className="text-sm font-medium" style={{ color: PALETTE.text }}>Digite o código recebido</p>
            <p className="text-xs" style={{ color: PALETTE.textDim }}>Enviamos um código para {recEmailMascarado}</p>
            <Field label="Código">
              <input className={inputCls} style={inputStyle} value={recCodigo} onChange={(e) => setRecCodigo(e.target.value)} placeholder="000000" />
            </Field>
            {recErro && <p className="text-sm" style={{ color: PALETTE.danger }}>{recErro}</p>}
            <Btn onClick={confirmarCodigo} disabled={recCarregando} className="w-full">{recCarregando ? "Verificando..." : "Confirmar código"}</Btn>
            <button onClick={voltarParaLogin} className="w-full text-center text-xs" style={{ color: PALETTE.textDim }}>Voltar ao login</button>
          </div>
        )}
        ...
        {modo === "esqueci-senha" && (
          <div className="rounded-2xl p-6 space-y-4" style={{ background: PALETTE.card, border: `1px solid ${PALETTE.cardBorder}` }}>
            <p className="text-sm font-medium" style={{ color: PALETTE.text }}>Defina sua nova senha</p>
            <Field label="Nova senha"><input type="password" className={inputCls} style={inputStyle} value={recNovaSenha} onChange={(e) => setRecNovaSenha(e.target.value)} /></Field>
            <Field label="Confirmar nova senha"><input type="password" className={inputCls} style={inputStyle} value={recConfirmar} onChange={(e) => setRecConfirmar(e.target.value)} /></Field>
            {recErro && <p className="text-sm" style={{ color: PALETTE.danger }}>{recErro}</p>}
            <Btn onClick={salvarNovaSenha} disabled={recCarregando} className="w-full">{recCarregando ? "Salvando..." : "Salvar nova senha"}</Btn>
          </div>
        )}

        {modo === "esqueci-ok" && (
          <div className="rounded-2xl p-6 space-y-4 text-center" style={{ background: PALETTE.card, border: `1px solid ${PALETTE.cardBorder}` }}>
            <p className="text-sm" style={{ color: PALETTE.ok }}>Senha alterada com sucesso!</p>
            <Btn onClick={voltarParaLogin} className="w-full">Fazer login</Btn>
          </div>
        )}
      </div>

      <p className="text-xs mt-8 tracking-wide" style={{ color: PALETTE.textDim }}>
        MARACUJÁ VITÓRIO · sistema interno
      </p>
    </div>
  );
}
...
function TrocarSenhaModal({ currentUser, onClose }) {
  ...
  const salvar = async () => {
    ...
    if (!novaSenha || novaSenha.length < 6) return setErro("A nova senha precisa ter pelo menos 6 caracteres.");
    if (novaSenha !== confirmar) return setErro("A confirmação não é igual à nova senha.");

    setSalvando(true);
    // Reautentica com a senha atual para confirmar que é realmente a pessoa dona da conta.
    const { error: erroLogin } = await supabase.auth.signInWithPassword({
      email: currentUser.email_recuperacao,
      password: senhaAtual,
    });
    if (erroLogin) {
      setSalvando(false);
    ...
    setSalvando(false);
    if (erroUpdate) return setErro("Não foi possível salvar a nova senha.");
    setSucesso(true);
  };
      ...
      <div className="w-full max-w-sm rounded-2xl p-5" style={{ background: PALETTE.card }}>
        ...
        {sucesso ? (
          ...
          <div className="space-y-3">
            ...
            <Btn onClick={salvar} disabled={salvando} className="w-full mt-1"><KeyRound size={16} /> {salvando ? "Salvando..." : "Salvar nova senha"}</Btn>
          </div>
        )}
      </div>
    </div>
...
// ---------- USUÁRIOS ----------
function UsuariosTab({ profiles, recarregarProfiles, currentUser }) {
  const [form, setForm] = useState({ nome: "", username: "", senha: "", cargo: "", emailRecuperacao: "", role: "funcionario" });
  const [criando, setCriando] = useState(false);
  const [erroForm, setErroForm] = useState("");
  const [aberto, setAberto] = useState(null);
  const [editForm, setEditForm] = useState({ cargo: "", emailRecuperacao: "" });
  const [novaSenhaAdmin, setNovaSenhaAdmin] = useState("");
  const [salvando, setSalvando] = useState(false);

  const add = async () => {
    setErroForm("");
    if (!form.nome || !form.username || !form.senha || !form.emailRecuperacao) {
      return setErroForm("Preencha nome, usuário, senha e e-mail de recuperação.");
    }
    setCriando(true);
    const resultado = await criarUsuario(form);
    setCriando(false);
    if (resultado.error) return setErroForm(resultado.error);
    setForm({ nome: "", username: "", senha: "", cargo: "", emailRecuperacao: "", role: "funcionario" });
    await recarregarProfiles();
  };

  const abrirEdicao = (u) => {
    setAberto(aberto === u.id ? null : u.id);
    setEditForm({ cargo: u.cargo || "", emailRecuperacao: u.email_recuperacao || "" });
    setNovaSenhaAdmin("");
  };

  const salvarEdicao = async (u) => {
    setSalvando(true);
    const resultado = await atualizarUsuario({ id: u.id, cargo: editForm.cargo, emailRecuperacao: editForm.emailRecuperacao });
    setSalvando(false);
  ...
  const alternarStatus = async (u) => {
    ...
    const resultado = await atualizarUsuario({ id: u.id, status: novoStatus });
    if (resultado.error) return alert(resultado.error);
    await recarregarProfiles();
  };

  const forcarNovaSenha = async (u) => {
    if (!novaSenhaAdmin || novaSenhaAdmin.length < 6) return alert("A senha precisa ter pelo menos 6 caracteres.");
    setSalvando(true);
    const resultado = await resetarSenhaUsuario(u.id, novaSenhaAdmin);
    setSalvando(false);
    if (resultado.error) return alert(resultado.error);
    setNovaSenhaAdmin("");
    ...
    <div className="space-y-5">
      <Card title="Novo usuário" icon={Users}>
        <div className="grid grid-cols-2 gap-3">
          ...
          <Field label="Senha"><input type="password" className={inputCls} style={inputStyle} value={form.senha} onChange={(e) => setForm((f) => ({ ...f, senha: e.target.value }))} /></Field>
          <Field label="Cargo"><input className={inputCls} style={inputStyle} value={form.cargo} onChange={(e) => setForm((f) => ({ ...f, cargo: e.target.value }))} /></Field>
          <Field label="E-mail de recuperação"><input type="email" className={inputCls} style={inputStyle} value={form.emailRecuperacao} onChange={(e) => setForm((f) => ({ ...f, emailRecuperacao: e.target.value }))} /></Field>
          <Field label="Função">
        ...
        {erroForm && <p className="text-sm mt-2" style={{ color: PALETTE.danger }}>{erroForm}</p>}
        <Btn onClick={add} disabled={criando} className="mt-3"><Plus size={16} /> {criando ? "Criando..." : "Criar usuário"}</Btn>
      </Card>

      <Card title="Usuários cadastrados">
        <div className="space-y-2">
          {profiles.map((u) => {
            const online = estaOnline(u.last_seen_at);
            const expandido = aberto === u.id;
            return (
              <div key={u.id} className="rounded-lg overflow-hidden" style={{ background: PALETTE.bg }}>
                <button onClick={() => abrirEdicao(u)} className="w-full flex items-center justify-between px-3 py-2 text-left">
                  <div>
                    <p className="text-sm font-medium flex items-center gap-1.5" style={{ color: PALETTE.text }}>
                      <span className="inline-block w-2 h-2 rounded-full" style={{ background: online ? PALETTE.ok : PALETTE.textDim }} />
                      {u.nome} <span style={{ color: PALETTE.textDim }}>· @{u.username}</span>
                      {u.status === "inativo" && <span className="text-xs px-1.5 py-0.5 rounded-full" style={{ background: PALETTE.dangerSoft, color: PALETTE.danger }}>inativo</span>}
                    </p>
                    <p className="text-xs" style={{ color: PALETTE.textDim }}>
                      {u.role === "admin" ? "Dono" : (u.cargo || "Funcionário")} · {online ? "online agora" : u.last_seen_at ? `visto por último ${formatarDataHora(u.last_seen_at)}` : "nunca acessou"}
                      {u.last_device && ` · ${u.last_device}`}
                    </p>
                  </div>
                  <ChevronRight size={16} style={{ color: PALETTE.textDim, transform: expandido ? "rotate(90deg)" : "none", transition: "transform .15s" }} />
                </button>

                {expandido && (
                  <div className="px-3 pb-3 space-y-3">
                    <div className="grid grid-cols-2 gap-3">
                      <Field label="Cargo"><input className={inputCls} style={inputStyle} value={editForm.cargo} onChange={(e) => setEditForm((f) => ({ ...f, cargo: e.target.value }))} /></Field>
                      <Field label="E-mail de recuperação"><input type="email" className={inputCls} style={inputStyle} value={editForm.emailRecuperacao} onChange={(e) => setEditForm((f) => ({ ...f, emailRecuperacao: e.target.value }))} /></Field>
                    </div>
                    <Btn variant="secondary" onClick={() => salvarEdicao(u)} disabled={salvando} className="w-full">{salvando ? "Salvando..." : "Salvar alterações"}</Btn>

                    <div className="rounded-md p-3 space-y-2" style={{ background: PALETTE.card, border: `1px solid ${PALETTE.cardBorder}` }}>
                      <p className="text-xs uppercase tracking-wide" style={{ color: PALETTE.textDim }}>Redefinir senha (define uma nova sem precisar da atual)</p>
                      <input type="password" className={inputCls} style={inputStyle} value={novaSenhaAdmin} onChange={(e) => setNovaSenhaAdmin(e.target.value)} placeholder="Nova senha (mín. 6 caracteres)" />
                      <Btn onClick={() => forcarNovaSenha(u)} disabled={salvando} className="w-full"><KeyRound size={14} /> Redefinir senha</Btn>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      <button
                        ...
                        className="text-xs px-2 py-1 rounded-md inline-flex items-center gap-1"
                        style={{ background: u.status === "ativo" ? PALETTE.okSoft : PALETTE.dangerSoft, color: u.status === "ativo" ? PALETTE.ok : PALETTE.danger }}
                      >
                        {u.status === "ativo" ? <Check size={12} /> : <X size={12} />} {u.status === "ativo" ? "Ativo — clique para desativar" : "Inativo — clique para ativar"}
                      </button>
                      {u.username !== "ednilson" && (
                        <Btn variant="danger" onClick={() => remover(u)}><Trash2 size={14} /> Excluir</Btn>
                      )}
                    </div>
                  </div>
                )}
...
export default function App() {
  ...
  // Busca a lista completa de usuários (só funciona para admin — a RLS bloqueia
  // o resto). Usada pela aba Usuários e para saber quem está online.
  const recarregarProfiles = async () => {
    const { data } = await supabase.from("profiles").select("*").order("nome");
    setProfiles(data || []);
  };

  // Carrega os dados do app + tenta restaurar a sessão salva no navegador.
  useEffect(() => {
    (async () => {
      ...
      if (sessao?.session?.user) {
        const perfil = await fetchOwnProfile(sessao.session.user.id);
        if (perfil && perfil.status === "ativo") {
          setCurrentUser(perfil);
          setTab(perfil.role === "admin" ? "dashboard" : "colheita");
        }
      }
      setLoading(false);
    })();

    // Mantém a sessão sincronizada (ex: token renovado sozinho em segundo plano).
  ...
  // Sinaliza presença (online/offline) enquanto a pessoa estiver com o app aberto.
  useEffect(() => {
    if (!currentUser) return;
    atualizarPresenca(currentUser.id);
    const intervalo = setInterval(() => atualizarPresenca(currentUser.id), 60000);
    return () => clearInterval(intervalo);
  }, [currentUser?.id]);

  // Admin: mantém a lista de usuários atualizada para ver quem está online.
  useEffect(() => {
    if (currentUser?.role === "admin") {
      recarregarProfiles();
      const intervalo = setInterval(recarregarProfiles, 30000);
      return () => clearInterval(intervalo);
    }
  }, [currentUser?.role]);
  ...
  // Chamado pela tela de login depois que o Supabase confirma usuário+senha.
  const handleLoggedIn = async () => {
    ...
    setCurrentUser(perfil);
    setTab(perfil.role === "admin" ? "dashboard" : "colheita");
    await registrarLogin(perfil.id);
    const novoAcesso = { id: uid(), username: perfil.username, nome: perfil.nome, dataHora: new Date().toISOString() };
    ...
    <div className="min-h-screen" style={{ background: PALETTE.bg }}>
      <header className="flex items-center justify-between px-5 py-4 border-b" style={{ background: PALETTE.card, borderColor: PALETTE.cardBorder }}>
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={sair} className="flex items-center gap-1 text-sm" style={{ color: PALETTE.textDim }}>
            <LogOut size={14} /> Sair
          </button>
        </div>
      </header>
      ...
      <main className="p-5 max-w-2xl mx-auto">
        ...
        {tab === "usuarios" && isAdmin && <UsuariosTab profiles={profiles} recarregarProfiles={recarregarProfiles} currentUser={currentUser} />}
        {tab === "acessos" && isAdmin && <AcessosTab acessos={acessos} />}
        {tab === "meusalario" && !isAdmin && <MeuSalarioTab pagamentos={pagamentosEder} />}
      </main>

      {mostrarTrocarSenha && (
        <TrocarSenhaModal currentUser={currentUser} onClose={() => setMostrarTrocarSenha(false)} />
      )}
    </div>
  );
}
