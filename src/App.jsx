...
function LoginScreen({ users, onLogin }) {
  ...
  // Pede explicitamente ao navegador para guardar essas credenciais.
  // Isso é necessário porque este app é uma SPA: nunca acontece uma navegação
  // de página de verdade, e é exatamente essa navegação que o Safari usa como
  // "gatilho" natural para oferecer salvar a senha. A Credential Management
  // API resolve isso — o Chrome simplesmente ignora a chamada (sem problema),
  // então isso só afeta o Safari/iCloud Keychain.
  const salvarCredencialNoNavegador = async () => {
    try {
      ...
      // Se o navegador não suportar ou o usuário recusar, apenas ignora.
    }
  };

  // Pequeno delay proposital: dá a sensação de uma autenticação real
  // acontecendo (mesmo sendo uma checagem local) em vez de um "flash" instantâneo.
  const trySubmit = (e) => {
    e.preventDefault();
    if (entrando) return;
    setEntrando(true);
    setTimeout(async () => {
      ...
      if (found) {
        ...
        onLogin(found);
        // não reseta "entrando": a tela vai trocar de qualquer forma
      } else {
        setEntrando(false);
        dispararErro("Usuário ou senha incorretos.");
      }
    }, 500);
  };
  ...
  return (
    ...
    >
      {/* animação de "shake" usada só na mensagem de erro do login */}
      <style>{`
        @keyframes shakeErro {
      ...
      <div className="w-full max-w-sm">
        ...
        {/* Formulário de verdade (com autoComplete correto) — é isso que faz o
            Chrome/Google Password Manager e o Safari/iCloud Keychain oferecerem
            salvar o usuário e a senha automaticamente. */}
        <form
          ref={formRef}
          onSubmit={trySubmit}
          autoComplete="on"
        ...
        >
          ...
          {err && (
            <p key={err + String(shakeErr)} className={"text-sm " + (shakeErr ? "shake-erro" : "")} style={{ color: PALETTE.danger }}>
              {err}
            </p>
          )}
          <Btn type="submit" disabled={entrando} className="w-full">
            {entrando ? (
              <>
                <span
                  className="spinner-entrar"
                  style={{ width: 14, height: 14, borderRadius: "50%", border: `2px solid ${PALETTE.polpaText}55`, borderTopColor: PALETTE.polpaText, display: "inline-block" }}
                />
                Entrando...
              </>
            ) : (
