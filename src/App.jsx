...
function ColheitaTab({ colheitas, setColheitas, precoKg, setPrecoKg, isAdmin, currentUser }) {
  ...
  return (
    <div className="space-y-5">
      ...
      {historico.length > 1 && (
        <Card title="Baldes por semana (safra)" icon={TrendingUp}>
          <div style={{ width: "100%", height: 220 }}>
            <ResponsiveContainer>
              <BarChart data={dadosGrafico} margin={{ top: 5, right: 5, left: -20, bottom: 5 }}>
                ...
                <Tooltip
                  formatter={(value, name) => [name === "baldes" ? `${value} baldes` : `${value} kg`, name === "baldes" ? "Baldes" : "Kg"]}
                  labelFormatter={(label) => `Semana de ${label}`}
                  contentStyle={{ fontSize: 12, borderRadius: 8 }}
                />
                <Bar dataKey="baldes" fill={PALETTE.polpa} radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </Card>
      ...
      <Card title="Entregas">
        ...
        <div className="space-y-2">
          {entregas.slice().reverse().map((e) => {
            ...
            const corLinha = vencida ? PALETTE.dangerSoft : venceEmBreve ? "#FDF3E3" : PALETTE.bg;
            return (
              <div key={e.id} className="rounded-lg overflow-hidden" style={{ background: corLinha }}>
                <button onClick={() => setEntregaAberta(aberta ? null : e.id)} className="w-full text-left px-3 py-2">
                  ...
                  <p className="text-xs" style={{ color: PALETTE.textDim }}>
                    {baldesResolvidos.length} baldes · {kg(kgEntrega)} kg · {money(e.valor)} · entregue em {e.dataEntrega}
                    ...
                    <div className="grid grid-cols-2 gap-2 text-sm mb-3">
                      ...
                      {(e.prazoData || e.prazo) && (
                        <p style={{ color: PALETTE.textDim }}>
                          Vencimento: <span style={{ color: PALETTE.text }}>{e.prazoData || e.prazo}</span>
                    ...
                    {(e.pagamentos || []).length > 0 && (
                      <div className="mb-3">
                        ...
                        <div className="space-y-1">
                          {e.pagamentos.map((p) => (
                            <div key={p.id} className="flex items-center justify-between text-sm rounded-md px-3 py-1.5" style={{ background: PALETTE.card }}>
